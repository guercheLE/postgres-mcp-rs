...
RUN cargo build --locked --release --bin postgresql-mcp-populate-embeddings
...
RUN ./target/release/postgresql-mcp-populate-embeddings --all
...
COPY --from=builder /app/target/release/postgresql-mcp ./postgresql-mcp
COPY --from=builder /app/target/release/postgresql-mcp-healthcheck ./postgresql-mcp-healthcheck
COPY --from=builder /app/mcp_store*.db.zst ./

HEALTHCHECK --interval=30s --timeout=5s --retries=3 CMD ["./postgresql-mcp-healthcheck"]
