...
use postgres_mcp::core::component_registry::ComponentRegistry;
use postgres_mcp::core::config_manager::load_config;
use postgres_mcp::core::config_schema::Transport;
use postgres_mcp::core::health_check_manager::HealthCheckManager;
use postgres_mcp::core::logger::init_logging;
use postgres_mcp::core::mcp_server::{McpifyServer, connect_stdio};
use postgres_mcp::core::otel;
use postgres_mcp::core::shutdown_handler::{install_shutdown_handlers, on_shutdown};
use postgres_mcp::data::store::{open_store, resolve_store_path};
use postgres_mcp::http::server::start_http_server;
use postgres_mcp::http::types::HttpServerConfig;

#[derive(Parser)]
#[command(
    name = "postgresql-mcp",
    about = "PostgreSQL 18.4 catalog MCP server",
    version
)]
struct Cli {
    #[command(subcommand)]
...
#[derive(Subcommand)]
enum Command {
    /// Interactively configure PostgreSQL 18.4 catalog credentials and connection settings
    Setup,
    ...
    /// Validate arguments, invoke the live PostgreSQL operation, and validate the response
    Call {
        operation_id: String,
        #[arg(short = 'a', long, default_value = "{}")]
        args: String,
    },
    /// Execute one parameterized PostgreSQL statement using bound values
    ExecuteSql {
        sql: String,
        /// JSON array of values bound as $1, $2, ... with PostgreSQL TEXT wire type
        #[arg(short = 'p', long, default_value = "[]")]
        parameters: String,
        /// Maximum rows returned for row-producing statements
        #[arg(long)]
    ...
    /// Verify the configured PostgreSQL connection URL and credentials
    TestConnection,
...
async fn run_harness_server() -> anyhow::Result<()> {
    ...
    let (otel_layer, otel_provider) = match otel::build_layer("postgresql-mcp") {
...
async fn main() -> anyhow::Result<()> {
    // Must run before anything builds a TLS client (the PostgreSQL connector
    ...
    let result = match cli.command {
        ...
        Command::Http {
            ...
            // spawns any concurrent work that would also touch env vars.
            unsafe {
                std::env::set_var("POSTGRES_MCP_TRANSPORT", "http");
                if let Some(host) = &host {
                    std::env::set_var("POSTGRES_MCP_HOST", host);
                }
                if let Some(port) = port {
                    std::env::set_var("POSTGRES_MCP_PORT", port.to_string());
                }
                if let Some(cors_allow) = &cors_allow {
                    std::env::set_var("POSTGRES_MCP_CORS_ALLOW", cors_allow);
                }
            }
            run_harness_server().await
        }
