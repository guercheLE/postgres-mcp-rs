...
/// pre-flight check only: the *actual* PostgreSQL credential is extracted
/// independently, per JSON-RPC call, inside
...
/// local config/keychain (see `AuthManager::postgres_credentials`), so a
...
async fn auth_gate(
    ...
    if !relaxed && extract_request_credentials(&headers).is_err() {
        return (
            StatusCode::UNAUTHORIZED,
            Json(json!({ "error": "missing or invalid PostgreSQL username/password" })),
        )
            .into_response();
    }
...
mod tests {
    ...
    async fn health_metrics_auth_gate_cors_and_server_bootstrap_are_exercised() {
        ...
        assert_eq!(
            gated
                .clone()
                .oneshot(request_from(remote, Some("Basic YWxpY2U6c2VjcmV0")))
                .await
                .unwrap()
                .status(),
        ...
        .unwrap();
        let auth_manager = Arc::new(TokioMutex::new(AuthManager::new(AuthMethod::Password)));
