...
use crate::tools::execute_sql_tool::execute_sql;
...
fn default_sql_parameters() -> Vec<serde_json::Value> {
    Vec::new()
}

#[derive(Debug, Deserialize, schemars::JsonSchema)]
pub struct ExecuteSqlArgs {
    /// Exactly one PostgreSQL statement. Use $1, $2, ... placeholders for
    /// every caller-controlled value; never concatenate values into SQL.
    pub sql: String,
    /// Values bound separately through PostgreSQL's extended query protocol.
    /// Parameters have TEXT wire type; cast non-text values explicitly, for
    /// example `$1::text::integer` or `$2::text::uuid`.
    #[serde(default = "default_sql_parameters")]
    pub parameters: Vec<serde_json::Value>,
    /// Maximum rows returned by SELECT/WITH/VALUES/TABLE statements.
    pub max_rows: Option<usize>,
}

/// Shared state every MCP tool method needs. `Clone`
...
impl McpifyServer {
    ...
    /// semantics (one PostgreSQL password credential source per process).
    ...
    #[tool(
        description = "Semantic search for PostgreSQL 18.4 catalog operations using a natural-language query."
    )]
    async fn search(
        &self,
    ...
    #[tool(
        description = "Return the schema, path, method, and documentation for a specific PostgreSQL 18.4 catalog operationId."
    )]
    ...
    #[tool(
        description = "Validate arguments, invoke a live PostgreSQL 18.4 catalog operation, and validate the response."
    )]
    async fn call(
        &self,
        ...
        let request_credentials = context
            ...
            .and_then(|parts| extract_request_credentials(&parts.headers).ok());
        ...
        self.run_tool("call", async move {
            ...
            let credentials = auth_manager
                .lock()
                .await
                .postgres_credentials(config.transport, request_credentials.as_ref())
                .await?;
            call_operation(
                &endpoint,
                &config,
    ...
    #[tool(
        description = "Execute one parameterized PostgreSQL statement over the native protocol. Values are bound separately from SQL to prevent SQL injection. Read and write statements are allowed; PostgreSQL roles and grants enforce authorization."
    )]
    async fn execute_sql(
        ...
        self.run_tool("execute_sql", async move {
            let credentials = auth_manager
                .lock()
                .await
                .postgres_credentials(config.transport, request_credentials.as_ref())
...
impl McpifyServer {
    ...
    /// and error handling, so each tool only implements
...
mod tests {
    ...
    fn argument_defaults_match_the_public_tool_contract() {
        ...
        let sql: ExecuteSqlArgs = serde_json::from_value(serde_json::json!({
            "sql": "SELECT 1"
        }))
        .unwrap();
        assert!(sql.parameters.is_empty());
        assert_eq!(sql.max_rows, None);
    }
    ...
    async fn search_and_get_return_mcp_content_envelopes() {
        ...
        let operation_id = {
            let conn = cached_store_connection("18").unwrap();
    ...
    #[tokio::test]
    async fn mcp_protocol_routes_all_four_tools() {
