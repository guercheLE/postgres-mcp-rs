...
const CONFIG_DIR_NAME: &str = ".postgresql-mcp";
const LOCAL_CONFIG_FILE: &str = "postgresql-mcp.config.yml";
...
fn parse_env_value(config_key: &str, value: &str) -> Value {
    if matches!(config_key, "timeout_ms" | "port")
...
/// (`./postgresql-mcp.config.yml`) -> home file
/// (`~/.postgresql-mcp/config.yml`) -> system file
/// (`/etc/postgresql-mcp/config.yml`) -> install-dir file -> built-in
...
pub fn load_config(cli_flags: Map<String, Value>) -> Result<Config, McpifyError> {
    ...
    let layers = [
        ...
        read_yaml_if_exists(&PathBuf::from("/etc/postgresql-mcp/config.yml")),
...
mod tests {
    ...
    fn base_flags() -> Map<String, Value> {
        json!({ "url": "postgresql://db.example/app", "auth_method": "password" })
            .as_object()
            .unwrap()
            .clone()
    }
    ...
    #[test]
    fn rejects_non_postgresql_auth_method_names() {
        let mut flags = base_flags();
        flags.insert("auth_method".to_string(), json!("basic"));

        let err = load_config(flags).unwrap_err();
        assert_eq!(err.code(), "CONFIGURATION_ERROR");
    ...
    fn numeric_environment_overrides_are_typed_before_deserialization() {
        ...
        assert_eq!(
            parse_env_value("url", "postgresql://db.example/app"),
            json!("postgresql://db.example/app")
        );
        assert_eq!(
