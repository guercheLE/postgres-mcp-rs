...
fn generated_server(args: &[&str]) -> Output {
    Command::new(env!("CARGO_BIN_EXE_postgresql-mcp"))
        ...
        // (`~/.postgresql-mcp/config.yml`, resolved via
...
fn version_help_versions_and_config_are_available() {
    ...
    assert!(stdout(&versions).contains("18"));
...
fn search_and_get_exercise_the_embedded_operation_catalog() {
    ...
    let get = generated_server(&["get", "read_pg_catalog_pg_stat_activity_747a66a052"]);
    assert!(get.status.success(), "{}", stderr(&get));
    let operation: serde_json::Value = serde_json::from_slice(&get.stdout).unwrap();
    assert_eq!(
        operation["operation_id"],
        "read_pg_catalog_pg_stat_activity_747a66a052"
    );
}
...
fn stdio_and_helper_binaries_cover_their_bootstrap_and_error_paths() {
    ...
    let healthy = Command::new(env!("CARGO_BIN_EXE_postgresql-mcp-healthcheck"))
    ...
    let unhealthy = Command::new(env!("CARGO_BIN_EXE_postgresql-mcp-healthcheck"))
        .current_dir(empty.path())
        .output()
        .unwrap();
    assert!(!unhealthy.status.success());

    let populate = Command::new(env!("CARGO_BIN_EXE_postgresql-mcp-populate-embeddings"))
    ...
    .unwrap();
    let populated = Command::new(env!("CARGO_BIN_EXE_postgresql-mcp-populate-embeddings"))
        .arg(&store_copy)
        .output()
        .unwrap();
...
fn http_command_serves_health_and_shuts_down_cleanly() {
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_postgresql-mcp"))
        .args([
            "http",
            "--host",
            "127.0.0.1",
