...
export POSTGRES_MCP_URL="${POSTGRES_MCP_URL:-http://127.0.0.1}"

export POSTGRES_MCP_AUTH_METHOD="${POSTGRES_MCP_AUTH_METHOD:-password}"


profile_query="${PROFILE_QUERY:-test query}"
profile_warm_seconds="${PROFILE_WARM_SECONDS:-30}"

cargo build --release --bin postgresql-mcp
cargo build --release --example profile_search

samply record --save-only --unstable-presymbolicate -o profile/cold-start.json.gz -- \
  ./target/release/postgresql-mcp search "$profile_query"
...
ready_file="$(mktemp "${TMPDIR:-/tmp}/postgresql-mcp-profile-ready.XXXXXX")"
rm -f "$ready_file"
warm_pid=""
samply_pid=""
cleanup() {
